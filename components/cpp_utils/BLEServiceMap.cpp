...
void BLEServiceMap::setByUUID(BLEUUID uuid,
	...
	ESP_LOGE("TAG_LOG", "MAP SERVICE UUID: %s, with uuid: %s", m_uuidMap.at(uuid.toString().c_str())->getUUID().toString().c_str(), service->getUUID().toString().c_str());
	ESP_LOGE("TAG_LOG", "MAP SERVICE UUID128: %s, with uuid128: %s", uuid.to128().toString().c_str(), service->getUUID().to128().toString().c_str());
} // setByUUID
